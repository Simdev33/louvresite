...
export default function AdminDisclaimerSitePage() {
    const { t } = useI18n();
    const [disclaimerSite, setDisclaimerSite] = useState<Record<string, string>>({});
    const [loading, setLoading] = useState(true);
    const [saving, setSaving] = useState(false);
    const [message, setMessage] = useState('');

    useEffect(() => {
        fetch('/api/admin/disclaimer-site')
            .then((res) => res.json())
            .then((data) => setDisclaimerSite(data.disclaimerSite ?? {}))
            .catch(() => setDisclaimerSite({}))
            .finally(() => setLoading(false));
    }, []);

    const handleChange = (lang: string, value: string) => {
        setDisclaimerSite((prev) => ({
            ...prev,
            [lang]: value,
        }));
    };
    ...
    const handleSave = async () => {
        ...
        setMessage('');
        try {
            const res = await fetch('/api/admin/disclaimer-site', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ disclaimerSite }),
            });
            if (res.ok) {
    ...
    return (
        <main className={styles.main}>
            ...
            <Link href="/admin/disclaimer-site" className={`${styles.menuItem} ${styles.menuItemActive}`}>{t('admin.disclaimerSite') || 'Disclaimer Site'}</Link>
            ...
            <button onClick={handleLogout} className={styles.menuItem} style={{ textAlign: 'left', cursor: 'pointer', background: 'none', border: 'none', color: '#ef4444', fontWeight: 'bold', width: '100%', padding: '10px 15px', marginTop: '10px' }}>Logout</button>
                ...
                <section className={styles.content}>
                    <h1 className={styles.title}>{t('admin.disclaimerSite') || 'Disclaimer Site'}</h1>
                            ...
                            <div style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
                                {LANGUAGES.map((lang) => (
                                    <div key={lang.code}>
                                        ...
                                        <RichEditor
                                            value={disclaimerSite[lang.code] || ''}
