...
const RichEditor = dynamic(() => import('@/components/RichEditor').then((mod) => mod.RichEditor as any), { ssr: false }) as any;
...
export default function AdminGuidePage() {
    const { t } = useI18n();
    const [guideData, setGuideData] = useState<Record<string, string>>({});
    ...
    useEffect(() => {
        fetch('/api/admin/guide')
            .then((res) => res.json())
            .then((data) => setGuideData(data.content ?? {}))
            .catch(() => setGuideData({}))
        ...
        <main className={styles.main}>
            ...
            <button onClick={handleLogout} className={styles.menuItem} style={{ textAlign: 'left', cursor: 'pointer', background: 'none', border: 'none', color: '#ef4444', fontWeight: 'bold', width: '100%', padding: '10px 15px', marginTop: '10px' }}>Logout</button>
                ...
                <section className={styles.content}>
                    <h1 className={styles.title}>{t('admin.guide') || 'Guide to Visits'}</h1>
                    ...
                    <div style={{ background: '#fff', padding: '1.5rem', borderRadius: '8px', border: '1px solid #eee' }}>
                        {loading ? (
                            ...
                            <div style={{ display: 'flex', flexDirection: 'column', gap: '2rem' }}>
                                {LANGUAGES.map((lang) => (
                                    <div key={lang.code}>
                                        <label style={{ display: 'block', fontSize: '1.1rem', fontWeight: 'bold', color: '#111', marginBottom: '0.5rem' }}>
                                            {lang.label} ({lang.code})
                                        </label>
                                        <div style={{ border: '1px solid #d1d5db', borderRadius: '8px', overflow: 'hidden' }}>
                                            <RichEditor
                                                value={guideData[lang.code] || ''}
                                                onChange={(val: string) => handleChange(lang.code, val)}
                                                placeholder={`Add meg az útmutató szövegét (${lang.label})...`}
                                            />
                                        </div>
                                    </div>
                                ))}
